...
namespace MrKWatkins.DataStructuresAndAlgorithms.Tests
{
    public sealed class TopologicalOrderExtensionsTests
    {
        ...
        public void TopologicalOrder_throws_if_dependentOnSelector_returns_null()
            => Assert.Throws<ArgumentException>(() => BuildTestGraph().TopologicalOrder(node => null).ToList())
                .ParamName.Should().Be("dependentOnSelector");

        [Fact]
        public void TopologicalOrder_throws_if_key_selector_returns_null()
            => Assert.Throws<ArgumentException>(() => BuildTestGraph().TopologicalOrder(node => node.DependentOn, node => (string) null).ToList())
        ...
        private sealed class Node : IEnumerable<Node>
        {
            public Node(int id)
            {
                Id = id;
            ...
            [NotNull]
            public List<Node> DependentOn { get; } = new List<Node>();
